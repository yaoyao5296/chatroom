/**
 * Vercel Edge Function: Codespace 唤醒 + 跳转入口（作为 api/index.ts 自动被 Vercel 识别）
 *
 * 部署后访问 https://<project>.vercel.app/ → rewrite 到 /api/index → 命中本 handler
 * 逻辑：
...
export default async function handler(req: Request, ctx: any) {
  ...
  if (!PAT || !CODESPACE_NAME) {
    ...
    return new Response(page(5, '⚠ 未配置 GH_PAT / CODESPACE_NAME，请在 Vercel 环境变量里设置'), {
